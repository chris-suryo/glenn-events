...
2. Run **all migrations in order** via Settings → SQL Editor:
   ...
   - `016_proposed_update_group_label.sql` — adds `proposed_updates.group_label` (nullable text). Glenn tags each proposed update with the real-world component it belongs to (e.g. "Petal & Stem", "Cake", "DJ"); the Review panel groups same-labelled updates into one tile with a block-approve. Additive — until applied (or when a label is missing), Review falls back to the existing flat list, so the approve flow never regresses.
   - `017_calendar_dates.sql` — converts `events.event_date` and `tasks.due_date` from `timestamptz` to `date`. They are calendar dates (a day, no time-of-day); as `timestamptz` a naive day was stored as midnight-UTC and rendered a day early in the event's zone (smoke-test D5). The `USING (col AT TIME ZONE 'UTC')::date` cast recovers the intended day from existing rows. **Apply before testing timezone fixes.**
